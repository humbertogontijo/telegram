...
async function computeHash(algo, password) {
    ...
    const pbk = await new Promise((res, rej) => {
        globalCrypto.subtle.importKey("raw", hash, { name: "PBKDF2" }, false, ["deriveBits", "deriveKey"])
            .then((importKey) => {
            return globalCrypto.subtle.deriveKey({ name: "PBKDF2", hash: "SHA-512", iterations: 100000, salt: salt }, importKey, { name: "HMAC", hash: "SHA-256" }, true, ["sign"]);
        })
            .then((deriveKey) => {
            return globalCrypto.subtle.exportKey("raw", deriveKey);
        })
            .then((exportKey) => {
